...
async function getUsersController(_req:Request,res:Response):Promise<Response> {
    try {
        const user = new User();
        ...
        if(result.status){
            ...
            let response:any = (result.statusCode==404) ? res.status(404).json(result) : res.status(500).json(result)
            return response;
        }
 ...
 async function getUserByIdController(req:Request,res: Response):Promise<Response>{
...
async function postUserController(req:Request,res: Response):Promise<Response>{
...
export default {getUsersController ,getUserByIdController,postUserController,updateUserController,deleteUserController}